...
import type { PaginatedResult, Pagination, PaginationQuery } from "@common/types/pagination.dto";
...
type PaginationInput = Required<PaginationQuery>;
...
export const getPaginationFromQuery = (query: PaginationQuery): PaginationInput => {
  let size = query.size ?? PAGINATION_PAGE_SIZE_DEFAULT;
  if (isNaN(size)) {
